...
public class vp_FPController : vp_CharacterController
{
    ...
    /// 节气门逻辑，用于基于播放器(相机)前向矢量在任意方向上移动飞行控制器。
    /// 它可以用于观众的凸轮，零重力，水下游泳，喷气背包和超级英雄风格的飞行。
    /// </summary>
    ...
    /// 处理所有跳跃逻辑，包括脉冲跳跃、连续跳跃、自由飞行模式下的垂直运动以及使控制器停止与天花板接触
    /// </summary>
    protected virtual void UpdateJump()
	{
        ...
        // apply accumulated 'hold jump' force  施加累积的“保持跳跃”力
	...
	protected override void FixedMove()
	{
        ...
        // move on our own
        //Debug.Log("m_MoveDirection =" + m_MoveDirection+ " m_MoveDirection2=" + m_MoveDirection * Delta * Time.timeScale);        //Debug.Log("m_MoveDirection =" + m_MoveDirection+ " m_MoveDirection2=" + m_MoveDirection * Delta * Time.timeScale);
    ...
    /// 由于控制器在FixedUpdate中移动，而相机在Update中将存在明显的相机抖动。
    /// 此方法模拟控制器在Update中的移动，并存储相机要读取的平滑位置。
    /// </summary>
    ...
    /// 此方法计算控制器速度乘数。取决于地面坡度。在“MotorSlopeSpeed”1.0中，斜坡中的速度将保持与平坦地面上的速度大致相同。
    /// 低于或高于1的值将使控制器减速/加速，这取决于我们是在上坡还是在下坡
    /// </summary>

    ...
    public class UIManager : MonoBehaviour
    {
	    ...
	    public static UIManager Instance
        {
            get
            {
                if (_Instance == null)
                {
                    GameObject root = GameObject.Find("UIRoot/_ScriptMgr");
                    _Instance = root.AddComponent<UIManager>();
                }
                return _Instance;
            }
        }
	    ...
	    public void ShowUIForms(string uiFormName, Util.CommonDelegate<BaseUIForm> callBack = null)
        {
            //参数的检查
            ...
            LoadFormsToAllUIFormsCatch(uiFormName, (BaseUIForm baseUIForms) =>
            {
        ...
        /// <returns></returns>
        private void LoadFormsToAllUIFormsCatch(string uiFormsName, Util.CommonDelegate<BaseUIForm> form)
        {
	    ...
	    private void LoadUIForm(string uiFormName, Util.CommonDelegate<BaseUIForm> form)
        {
            ...
            //if (!string.IsNullOrEmpty(strUIFormPaths))
            {
                ...
                ResUtil.LoadPrefab(uiFormName, (string assetName, object go) =>
                {
                    GameObject g = go as GameObject;
                    goCloneUIPrefabs = GameObject.Instantiate<GameObject>(g);
                    ...
                    if (_TraCanvasTransfrom != null && goCloneUIPrefabs != null)
                    {
                        ...
                        if (baseUiForm == null)
                        {
                            Loger.Error("baseUiForm==null! ,请先确认窗体预设对象上是否加载了baseUIForm的子类脚本！ 参数 uiFormName=" + uiFormName);
                            form(null);
                            return;
                        }

                        switch (baseUiForm.GetUIType().UIForms_Type)
                        {
                        ...
                        _DicALLUIForms.Add(assetName, baseUiForm);
                        form(baseUiForm);
                    }
                    else
                    {
                        Loger.Error("_TraCanvasTransfrom==null Or goCloneUIPrefabs==null!! ,Plese Check!, 参数uiFormName=" + uiFormName);
                        form(null);
                    }
                });
            }
            //else
            //{
            //    Loger.Error("出现不可以预估的错误，请检查，参数 uiFormName=" + uiFormName);
            //    form(null);
            //}
        }//Mehtod_end
